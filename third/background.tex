...
\section{環境モデル}
実験を行う環境モデルについて述べる．環境モデルはNVIDIA Isaac Lab にて標準提供されているlocomotion（二足歩行ロボットが歩行動作＝タスク）を使用する．本モデルの外観を図Xに示す．
下に図を貼るG1とH1の環境
...
\section{状態・行動空間}
本学習モデルにおける観測空間（状態空間）および行動空間の定義は以下の通りである．

\subsection{観測空間}
\begin{itemize}
\item ロボットの位置と線形速度あとで式を書く
\item 目標となる線形速度と角速度
\item 各関節の速度
\item 各足の接触状態
\item 周囲環境（不整地環境のみ）ロボットの足元の地形形状を把握するための地形の高さ情報
\end{itemize}
\subsection{行動空間}
